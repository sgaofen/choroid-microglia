"""Per-cell audit for v30c outputs — same regions as v30b for comparison."""
import json, sys
...
RAW = ROOT / 'data/raw'
V30C = ROOT / 'experiments/v30c_snap_and_blob_gate'

sys.path.insert(0, str(V30C))
from v30c_run import normalize
...
def make_cell_crops(stem, y_lo, y_hi, x_lo, x_hi, tag, half=40, max_cells=15):
    ...
    norm = normalize(raw)
    recs = json.loads((V30C / f'{stem}_seeds_v30c.json').read_text())
    ...
    for i, r in enumerate(accepted):
        ...
        # mark old (pre-snap) center in red if it moved
        if 'yc_raw' in r and (r['yc_raw'] != r['yc'] or r['xc_raw'] != r['xc']):
            axes[1].plot(r['xc_raw'] - x0, r['yc_raw'] - y0, 'x',
                         ms=10, mec='red', mew=1.5)
        axes[1].set_title(
            f'{code} score={r["score"]:.2f} blob={r["blob"]:.2f} '
            f'tube={r["tube"]:.2f} fgd={r["fg_dens"]:.2f}', fontsize=8)
        axes[1].axis('off')
        fig.tight_layout()
        out = V30C / f'audit_{stem}_{tag}_cell{i:02d}_L{r["label"]}.png'
...
if __name__ == '__main__':
    ...
    for (s, y_lo, y_hi, x_lo, x_hi, tag, n) in targets:
        make_cell_crops(s, y_lo, y_hi, x_lo, x_hi, tag, max_cells=n)
