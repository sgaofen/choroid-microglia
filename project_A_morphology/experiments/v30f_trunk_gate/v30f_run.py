...
OUT = ROOT / 'experiments/v30f_trunk_gate'

# --- thresholds ---
SCORE_STRONG = 0.40
# v30f: SCORE_WEAK raised from 0.22 -> 0.26 per threshold sweep. At 0.26,
# the cells removed have low n_trunks at 23-38% rate vs ~0% at v30e baseline,
# i.e. tightening the score floor preferentially kills vessel pieces and
# process beads while preserving real microglia.
SCORE_WEAK   = 0.26
HESSIAN_SIGMA = 2.5
...
# v30f: FG_DENSITY_MIN demonstrated zero discriminative power in the sweep
# (varying 0.35 -> 0.65 changed cell counts by <10 per image because real
# soma neighborhoods are uniformly dense). Disabled.
FG_DENSITY_MIN = 0.0

# v30f: post-merge trunk gate. After resolve_seeds, each weak/low_confidence_soma
# must have at least this many annulus trunks (primary processes leaving the
# soma) to remain accepted. Strong cells are exempt because they already pass
# the n_dirs >= 3 strong-ok test, which is a related but stricter signal.
MIN_N_TRUNKS_WEAK = 1
TRUNK_ANNULUS_PAD_IN = 2   # r_in = r_soma + this
TRUNK_ANNULUS_WIDTH  = 6   # r_out - r_in
...
def annulus_trunk_count_seed(skel, yc, xc, r_in, r_out):
    """Count connected components of skeleton in annulus around (yc, xc).
    Matches primary_trunk_counter.annulus_trunk_count — duplicated here so
    v30f_run.py is self-contained."""
    H, W = skel.shape
...
def run_image(stem):
    ...
    print(f'  pre-trunk-gate: {dict(final)}')

    # v30f: trunk-gate post-pass. Weak / low_confidence_soma must show at
    # least one annulus trunk; strong cells are exempt because they passed
    # n_dirs >= 3 already.
    demoted = 0
    for s in seeds:
        if s['type'] not in ('weak', 'low_confidence_soma'):
            s['n_trunks'] = -1  # marker: not computed
            continue
        r_soma = float(dist_s[s['yc'], s['xc']])
        r_in = max(3, int(round(r_soma)) + TRUNK_ANNULUS_PAD_IN)
        r_out = r_in + TRUNK_ANNULUS_WIDTH
        n_trunks = annulus_trunk_count_seed(skel, s['yc'], s['xc'], r_in, r_out)
        s['n_trunks'] = int(n_trunks)
        s['r_in'] = int(r_in); s['r_out'] = int(r_out)
        if n_trunks < MIN_N_TRUNKS_WEAK:
            s['type'] = 'process_peak'
            s['demoted_v30f_trunk_gate'] = True
            demoted += 1
    print(f'  v30f trunk-gate demoted {demoted} weak/low_confidence cells '
          f'(n_trunks < {MIN_N_TRUNKS_WEAK})')
    ...
    (OUT / f'{stem}_seeds_v30f.json').write_text(json.dumps(safe_seeds, indent=1))
    (OUT / f'{stem}_endpoint_counts_v30f.json').write_text(
